...
const CallToAction = () => {
  ...
  return (
    ...
    >
      ...
      {/* Decorative shapes */}
      <div className="absolute top-0 left-1/4 w-[400px] h-[400px] rounded-full bg-gradient-to-br from-bakery-peach/30 to-transparent blur-3xl" />
      <div className="absolute bottom-0 right-1/4 w-[500px] h-[500px] rounded-full bg-gradient-to-tl from-bakery-pink-light/40 to-transparent blur-3xl" />

      {/* Floating accent dots */}
      <div
        className="absolute top-1/3 left-[15%] w-4 h-4 rounded-full bg-bakery-pink-dark/30"
        style={{ animation: "float 5s ease-in-out infinite" }}
      />
      <div
        className="absolute bottom-1/4 right-[20%] w-3 h-3 rounded-full bg-bakery-brown/40"
      ...
      <div className="container-custom relative z-10">
        <div className="max-w-4xl mx-auto">
          {/* Main content */}
          <div className="text-center mb-16 opacity-0 transition-all duration-1000 animate-fade-in-up">
            {/* Badge */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {actions.map((action, index) => (
              <Link
                key={action.title}
                href={action.href}
                className="group relative bg-white rounded-3xl p-8 shadow-lg hover:shadow-xl border border-bakery-pink-light/20 hover:border-bakery-pink-light/50 transition-all duration-500 hover:-translate-y-2 overflow-hidden opacity-0 animate-fade-in-up"
          ...
          <div
            className="mt-16 text-center opacity-0 transition-all duration-1000 animate-fade-in-up"
