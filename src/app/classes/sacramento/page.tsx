import Link from "next/link";
import Script from "next/script";
import FAQAccordion from "@/components/FAQAccordion";
import SectionDivider from "@/components/ui/sectionDivider";
import { buildPageMetadata } from "@/lib/metadata";

export async function generateMetadata() {
  return buildPageMetadata({
    title: "Cookie Decorating Classes in Sacramento, CA | Rose & Sugar",
    description:
      "Join cookie decorating classes near Sacramento, CA. Rose & Sugar offers hands-on instruction, small groups, and custom cookie artistry.",
    path: "/classes/sacramento",
  });
}

const faqs = [
  {
    question: "Do you offer cookie decorating classes for Sacramento?",
    answer:
      "Yes, Sacramento guests are welcome to join our small-group classes nearby.",
  },
  {
    question: "Are these hands-on cooking classes?",
    answer:
      "Yes. You will decorate your own cookie set with guided instruction.",
  },
  {
    question: "How long do the classes last?",
    answer:
      "Classes typically run about 1.5 to 2 hours depending on the theme.",
  },
  {
    question: "Do you take custom cookie orders in Sacramento?",
    answer:
      "Yes, we create custom cookie sets for Sacramento celebrations and events.",
  },
];

const SacramentoClassesPage = () => {
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: faqs.map((faq) => ({
      "@type": "Question",
      name: faq.question,
      acceptedAnswer: {
        "@type": "Answer",
        text: faq.answer,
      },
    })),
  };

  return (
    <div className="page-wrapper">
      <Script
        id="faq-jsonld-sacramento"
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(faqJsonLd) }}
      />
      <main className="page-content">
        <div className="container-custom">
          <h1 className="page-heading">
            Cookie Decorating Classes in Sacramento
          </h1>
          <p className="body-text text-center max-w-3xl mx-auto">
            Looking for cooking classes in Sacramento with a sweet twist? Rose
            &amp; Sugar hosts cookie decorating classes that blend creativity,
            technique, and celebration. You&apos;ll learn to decorate like a pro
            while enjoying a friendly, small-group experience.
          </p>

          <div className="mt-10 bg-white rounded-xl p-6 md:p-8 shadow-xl">
            <h2 className="section-subheading text-center">
              Hands-On Cookie Decorating for Sacramento Guests
            </h2>
            <div className="content-spacing mt-6">
              <p className="body-text">
                Our classes cover the essentials: outlining, flooding, and
                detail piping. We keep classes intimate to ensure you get
                helpful feedback and leave with cookies you&apos;re proud to
                share.
              </p>
              <p className="body-text">
                Need custom cookies in Sacramento for a birthday, shower, or
                business event? We create bespoke cookie sets to match your
                theme and vision.
              </p>
            </div>
          </div>

          <SectionDivider icon="cookie" />

          <div className="mt-10 bg-bakery-pink-light/30 rounded-xl p-6 md:p-8">
            <h2 className="section-subheading text-center">
              Reserve Your Sacramento-Area Class
            </h2>
            <p className="body-text text-center max-w-2xl mx-auto mt-4">
              See upcoming dates and book your seat in minutes.
            </p>
            <div className="mt-6 flex justify-center">
              <Link href="/classes" className="btn-primary">
                View Class Schedule
              </Link>
            </div>
          </div>

          <SectionDivider icon="cookie" />

          <section className="bg-white rounded-xl p-6 md:p-8 shadow-xl mt-10">
            <h2 className="section-subheading text-center">
              Order Custom or Ready-to-Order Cookies
            </h2>
            <p className="body-text text-center max-w-2xl mx-auto mt-4">
              Need custom cookies in Sacramento? Choose a bespoke design or
              browse pre-designed sets that are ready to order.
            </p>
            <div className="mt-6 flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/cookies/custom-orders" className="btn-primary">
                Start a Custom Order
              </Link>
              <Link href="/cookies/pre-designed" className="btn-secondary">
                Shop Pre-Designed Sets
              </Link>
            </div>
          </section>

          <SectionDivider icon="flower2" />

          <section className="bg-white rounded-xl p-6 md:p-8 shadow-xl mt-10">
            <h2 className="section-subheading text-center">
              Sacramento Cookie Class FAQs
            </h2>
            <FAQAccordion faqs={faqs} />
          </section>
        </div>
      </main>
    </div>
  );
};

export default SacramentoClassesPage;
