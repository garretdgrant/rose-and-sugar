...
Returned by list, get by id, create, and update invoice endpoints.
...
- `invoiceNumber` is generated server-side using a per-client sequence with the
  client's first word as the prefix (uppercased).
...
Get a single invoice by id (staff can access any invoice, clients can access
their own).
...
- Body:
  ...
  - `status` (`DRAFT | DUE | OVERDUE | PAID | VOID`, optional, default `DRAFT`)
